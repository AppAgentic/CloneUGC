...
For CloneUGC's short references, Gemini 3.8 Agentic Video is the production semantic-analysis default. Static processing is retained only as a benchmark control and an explicitly degraded fallback:
...
2. Run Gemini 3.8 Agentic Video across the complete clip to inventory shots, subjects, objects, text, audio, environment, beats, action choreography, identity/occlusion, camera motion, lighting, and secondary motion. It must inspect split-second gestures, periodic or fast motion, counting, speed ramps, freeze/reverse/loop hypotheses, and ambiguous transitions rather than reducing actions to generic pose labels. Gemini interprets detected cuts; deterministic probes remain the timestamp system of record. Fast subject movement must not be treated as sped-up playback without independent cues.
3. Keep static default, 5 FPS, and 10 FPS runs only as blinded Phase 0 controls. They measure the incremental value of Agentic Video and provide a degraded fallback when Agentic Video is unavailable; they do not determine the normal production route.
...
6. Persist the structured provider response and processing steps as a private evidence artifact before producing a bounded human-readable summary. The shared CLI still bounds displayed summaries at 6,000 UTF-8 bytes, but its evidence-output boundary now writes the complete provider request/response bytes to a new private `0600` artifact. CloneUGC validates and persists that lossless exchange; it never parses the bounded display summary back into a Fidelity Map.
...
8. If Agentic Video times out, is safety-blocked, or loses its interaction state, a static fallback may produce an explicitly degraded draft map that is ineligible for paid generation until the missing evidence is resolved. An expired file may be re-uploaded under a new lineage event; it must never be silently treated as the same provider interaction.
...
This routing is deliberate. Google's September 2026 Agentic Video release adds dynamic timeline navigation, adaptive frame rate and resolution, transcript/audio inspection, sub-second moment retrieval, anomaly detection, and action/object counting. Those capabilities map directly to reconstruction forensics, and CloneUGC's family-montage evidence showed Agentic Video recovering exact fan/ripple choreography that the prior static-derived map had collapsed into generic posing. Static processing remains useful as a controlled baseline and degraded fallback, not the production default.
...
- **Agentic production lane (schema name `hybrid_agentic`):** deterministic probes plus a complete Gemini 3.8 Agentic Video analysis. The legacy schema name is retained for evidence compatibility; this lane does not depend on a winning static pass.
...
The bake-off quantifies Agentic Video's incremental quality, latency, and cost but does not change the operator-approved production default. A material regression or unavailable Agentic service produces an explicit degraded/fail-closed state rather than silently switching a paid reconstruction to static-derived evidence. Three reference clips remain a deliberately thin thesis test, not a production-quality claim.
...
- Mission Control: the shared `mc video-analyze` Gemini 3.8 Flash path, Agentic-default analysis with explicit static fallback, streamed uploads, provider-expiry caching, stateful interactions, bounded concurrency and request timeouts, and persisted provenance.
