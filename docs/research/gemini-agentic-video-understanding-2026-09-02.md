...
The material event deltas were: first lockout +50 ms in the reconstruction, lowering start +1,400 ms, bottom turnaround +2,850 ms, and second upward start +2,950 ms. Native 60 fps in the original versus 24 fps in the reconstruction cannot explain wall-clock phase compression. The earlier analysis accepted generic gait/gravity/audio cues without measuring independent full-clip clocks. An initial `analysis-v3` correction required repeated-cycle and background-action counting, but that overfit this gym clip. `analysis-v4` instead asks Gemini to discover at least two independent temporal anchors from any available foreground, background, camera, physics, mechanical, particle/fluid, or audio motion; timestamp and measure each against plausible natural ranges; reconcile global versus local evidence; and abstain when fewer than two useful clocks exist. This correction supersedes the canary's playback conclusion while preserving its evidence that the adapter and schema functioned.

The exact `analysis-v4` prompt then failed the same raw-source canary: Gemini again sampled only about two apparent presses, constructed two plausible-sounding clocks from incomplete coverage, and returned `real_time`. This shows the miss is primarily adaptive under-sampling, not a lack of semantic categories, and makes raw single-pass `real_time` a no-ship input to the compiler.

A content-agnostic inspection experiment deterministically expanded both the accelerated source and natural-speed reconstruction to 4× duration without interpolation or audio. With the expansion factor disclosed and timestamps mapped back to source time, Gemini 3.8 recovered five full plus one partial foreground cycle and multiple accelerated background actions in the source, while returning natural speed for the reconstruction. The same discrimination held in both Agentic and cheaper static processing. Static provenance was 12.466 seconds / 4,119 tokens for the accelerated source and 11.057 seconds / 3,377 tokens for the natural control. This is one positive and one negative canary, not evidence of general accuracy.

Claude Code's independent review agreed that more event/anchor wording cannot repair missing temporal coverage. Its recommended Phase 0 direction is a deterministic temporal evidence pack (PTS, cut and duplicate/hash structure, plus a time-expanded inspection view), a low-cost static Gemini timing pass, Agentic escalation only for disagreement or suspected ramps, and fail-closed disputed/unknown playback when views conflict. Motion energy or frame hashes can locate structural anomalies such as freezes, loops, reversals, or candidate ramps; they must not independently claim global playback speed. Before changing the compiler contract, benchmark this lane on constructed 0.5×/1×/1.5×/2×/3×, ramp, freeze, reverse, and loop variants plus naturally fast real-time negative controls.

### Iterative speed benchmark and scalar-oracle rejection

A first deterministic corpus applied silent 0.5×, 1×, 1.5×, 2×, and 3× transforms to three permission-safe internal human-motion sources. This is a smoke corpus with three independent sources, not fifteen independent examples. The initial time-expanded static Gemini 3.8 lane committed on every case but scored 4/15 (26.7%): 3/3 real-time controls, 0/3 slowdowns, and 1/9 accelerations. The only acceleration detected was a 3× strength clip, estimated as 4×. Appending deterministic duplicate/displacement/blur measurements did not change the 1× default in a six-case pilot. Five-hypothesis sequential and simultaneous comparison views also failed. Agentic Video repeated the same error at substantially higher token cost.

An explicit hypothesis prompt exposed that Gemini's observation and verdict were inconsistent. On a known 3× reaction clip it measured several motions at about 0.2 seconds, yet assigned 95% likelihood to 1×. On a 4× time-expanded reaction clip it called a roughly 0.7-second blink natural. Prompting for likelihoods and counter-evidence did not correct the scalar verdict. Therefore Gemini must not be used as a playback-rate oracle.

The second Claude Code review agreed and identified benchmark corrections. Absolute rate from one silent final render is not universally identifiable: 1.5× can fall inside natural human variation, and motion energy is not comparable across source families. The benchmark must separate deterministic structural retiming, clock-bearing semantic cases, and observationally ambiguous cases; add interpolated and high-frame-rate slowdowns, retained-audio variants, and naturally fast/slow real-time negatives; use constant expansion windows rather than a class-correlated 30-second cap; score multiplier error in log2 space; and reward abstention in the ambiguous tier. Reconstruction fidelity should compare observed source/output tempo directly, which is identifiable even when the absolute label is unknown.

The replacement design separates responsibilities. A versioned Gemini observer prompt forbids speed labels and returns only timestamped, typed, directly observed events. A deterministic adjudicator maps inspection timestamps back to delivered time, scores configurable candidate multipliers against externally calibrated natural-duration priors, requires at least two independent clocks to agree, and fails closed when the likelihood margin is weak. The first live observer run on a known 2× strength case returned one directly observed full repetition but marked the background traversal inferred; the adjudicator therefore correctly has insufficient evidence rather than fabricating a 1× verdict. This validates the failure behavior, not detection accuracy. Natural-duration priors and the expanded corpus remain required before the analyzer can pass a detection gate.
