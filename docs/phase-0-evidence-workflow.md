...
`src/adapters/file-analysis-runtime.ts` supplies the crash-durable Phase 0 store. Ledger creation uses an exclusive atomic publication before the provider call, so concurrent duplicates cannot both submit. Terminal records are immutable, abandoned `running` records require reconciliation, and a restarted runner returns an already succeeded result without invoking the provider. Raw and structured artifacts are content-addressed, written with `0600` permissions and file/directory sync, and carry an immutable per-provenance receipt. Hash/path confusion and corrupt existing bytes fail closed.

The 2026-09-04 live canaries used the corrected Winter Arc source. The shared Agentic boundary returned a real interaction ID and eight adaptive processing calls/results; the full Agentic adapter-to-runner path returned a second real interaction with seven processing calls, wrote two independently hashed artifacts to its configured canary store before returning, marked the candidate eligible for Fidelity Map materialization, and correctly left `mayDrivePaidGeneration` false. Separate live 5fps static calls proved that Gemini accepts the exact sampling object and that the adapter preserves an explicit zero processing-call count; the runner correctly kept that static result ineligible for map materialization. A final Agentic canary wrote to the filesystem store, reopened under a new runner configured with an intentionally nonexistent provider executable, and returned the byte-identical completed record without a second call. These canaries prove the local transport, provenance, and restart rail, not the three-family quality gate or remote object-store wiring.
