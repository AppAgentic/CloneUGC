...
| P2 | Static-default, static-5fps, static-10fps, and Agentic production lanes, three repeats per clip, pinned exact model | The evidence-grade execution/provenance runner now exists, but the current CLI adapter cannot select true static 5/10 FPS sampling or return the complete untruncated provider interaction | Add the live Gemini adapter behind `AnalysisRunner`, then bind the already pinned `gemini-3.8-flash` calls to the standing in-scope authority; partial evidence remains rejected by `scoreBenchmarkCorpus` |
