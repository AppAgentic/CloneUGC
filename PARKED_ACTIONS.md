...
| P2 | Static-default, static-5fps, static-10fps, and hybrid analyzer lanes, three repeats per clip, pinned exact model | Requires an evidence-grade runner; the current CLI cannot select true static 5/10 FPS sampling or persist the complete untruncated structured provider response | Implement that runner against the already pinned `gemini-3.8-flash` adapter and bind each call to the standing in-scope authority; partial evidence remains rejected by `scoreBenchmarkCorpus` |
