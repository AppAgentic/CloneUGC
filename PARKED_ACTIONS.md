...
|---|---|---|---|
| P1 | Blind human annotations for the three benchmark clips (cuts, segment lengths, playback classes, action events, continuity, lighting, secondary motion, text, audio, rights risks) | Human work; must be authored before seeing model output | An annotator with the source clips and the annotation schema in `src/benchmark.ts` |
| P2 | Static-default, static-5fps, static-10fps, and hybrid analyzer lanes, three repeats per clip, pinned exact model | Paid Gemini calls over the network | Provider credentials in `mc-vault`, approved data-use terms, a budget approval, and the pinned model decision (3.7 versus 3.8) |
...
| P5 | Three blind scorers with structured per-dimension QA on each pair | Human work | P4 outputs and a scoring session using the `QAReport` schema in `src/qa.ts` |
